...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope discord-bot-cli
...
scope:** the default personal scope is **private** (`--scope discord-bot-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `discord-bot-cli`). Private records are served only to a recall in the same
