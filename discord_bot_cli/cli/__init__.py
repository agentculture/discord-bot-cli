"""Unified CLI entry point for discord-bot-cli.

The agent-first global verbs (``whoami``, ``learn``, ``explain``, ``overview``,
``doctor``) are registered here under :mod:`discord_bot_cli.cli._commands`,
alongside the ``cli`` noun group. Future noun groups register via their own
``register()`` functions following the same pattern.

Error propagation contract
--------------------------
Every handler raises :class:`discord_bot_cli.cli._errors.CliError` on
failure; ``main()`` catches it via :func:`_dispatch` and routes through
:mod:`discord_bot_cli.cli._output`. Unknown exceptions are wrapped into a
...
_ISSUES_URL = "https://github.com/agentculture/discord-bot-cli/issues"
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser = _CliArgumentParser(
        prog="discord-bot-cli",
        description="discord-bot-cli — a clonable template for AgentCulture mesh agents.",
    )
    parser.add_argument(
        "--version",
        action="version",
